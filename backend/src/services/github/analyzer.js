...
async function analyzeGitHubUser(username, options = {}) {
  ...
  // Optional limit to avoid timeouts (used for comparison mode)
  const maxRepos = Number.isFinite(options.maxRepos) ? options.maxRepos : null;
  const limitedRepos = maxRepos ? ownRepos.slice(0, maxRepos) : ownRepos;
  if (maxRepos && ownRepos.length > maxRepos) {
    console.log(
      `⚠️ Limiting repositories for analysis: ${maxRepos}/${ownRepos.length}`,
    );
  }

  // Analyze repositories (with batching to avoid rate limits)
  console.log(`📦 Analyzing ${limitedRepos.length} repositories...`);
  ...
  for (let i = 0; i < limitedRepos.length; i += batchSize) {
    ...
    console.log(
      `   Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(limitedRepos.length / batchSize)}...`,
    );
    ...
    const batchResults = await Promise.all(
      batch.map(async (repo) => {
        try {
          ...
          console.log(
            `   ✅ ${repo.name}: ${analyzed.commitCount || 0} commits`,
          );
          return analyzed;
        } catch (error) {
          console.error(
            `   ❌ Error analyzing repo ${repo.name}:`,
            error.message,
          );
  ...
  console.log(
    `✅ Analyzed ${repositories.length} repositories, total commits: ${totalCommits}`,
  );
  ...
  if (!yearValidation.valid) {
    console.error(
      "❌ Year breakdown validation failed:",
...
function calculateContributions(repositories, contributionCalendar = null) {
  ...
  const repoCommits = repositories.reduce(
    (sum, r) => sum + (r.commitCount || 0),
    0,
  );
  ...
  if (
    ...
    const commitsByMonth = groupCommitsByMonthFromCalendar(
      contributionCalendar.days,
    );

    // Calculate streaks from calendar
    const { longestStreak, currentStreak } = calculateStreaksFromCalendar(
      contributionCalendar.days,
    );
  ...
  const averageCommitsPerDay =
    daysSinceStart > 0 ? totalCommits / daysSinceStart : 0;
  ...
  sortedDays.forEach((day) => {
    ...
    if (day.count > 0) {
      if (
        prevDate &&
        Math.floor(
          (prevDate.getTime() - dayDate.getTime()) / (1000 * 60 * 60 * 24),
        ) === 1
      ) {
        // Consecutive day
        tempStreak++;
...
function inferSkills(repositories) {
  ...
  const skills = Array.from(skillMap.values()).map((skill) => {
    ...
    skill.level = calculateSkillLevel(
      skill.totalLines,
      skill.evidenceCount,
      daysSinceUse,
    );
...
function calculateStreaks(repositories) {
  ...
  if (daysSinceLastCommit <= 1) {
    ...
    for (let i = 1; i < sortedRepos.length; i++) {
      ...
      const daysDiff = Math.floor(
        (expectedDate.getTime() - repoDate.getTime()) / (1000 * 60 * 60 * 24),
      );
      if (daysDiff === 0) {
        currentStreak++;
        expectedDate.setDate(expectedDate.getDate() - 1);
  ...
  sortedRepos.forEach((repo) => {
    ...
    if (prevDate) {
      const daysDiff = Math.floor(
        (prevDate.getTime() - repoDate.getTime()) / (1000 * 60 * 60 * 24),
      );
      if (daysDiff <= 1) {
        tempStreak++;
      } else {
...
function findInactiveGaps(repositories) {
  ...
  for (let i = 1; i < sortedRepos.length; i++) {
    const daysBetweenUpdates = daysBetween(
      sortedRepos[i - 1].pushedAt,
      sortedRepos[i].pushedAt,
    );
...
function determineProjectFocus(repositories) {
  const activeRepos = repositories.filter(
    (r) => r.maturityStage === "active" || r.maturityStage === "stable",
  );
  const avgCommits =
    activeRepos.reduce((sum, r) => sum + (r.commitCount || 0), 0) /
    activeRepos.length;
...
function assessDocHabits(repositories) {
  const withDocs = repositories.filter(
    (r) => r.documentationQuality !== "none",
  ).length;
  ...
  const stats = Object.entries(langCount)
    .map(([name, count]) => {
      ...
      if (isNaN(percentage) || isNaN(bytesPercentage) || !name) {
        console.warn(`[Language Stats] Invalid entry detected:`, {
          name,
          count,
          bytes,
          percentage,
  ...
  if (contributions?.calendar && contributions.calendar.length > 0) {
    contributions.calendar.forEach((day) => {
      ...
      if (yearMap[year]) {
        ...
        const existingMonth = yearMap[year].monthlyCommits.find(
          (m) => m.month === monthKey,
        );
        if (existingMonth) {
  ...
  if (contributions?.calendar) {
    Object.keys(yearMap).forEach((year) => {
      ...
      if (yearDays.length > 0) {
        ...
        const sortedDays = [...yearDays].sort(
          (a, b) => new Date(b.date) - new Date(a.date),
        );
