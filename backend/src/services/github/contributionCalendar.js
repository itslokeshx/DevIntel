...
async function fetchContributionCalendar(username) {
  try {
    // Use a 1-year range (GitHub API limit for contributionsCollection)
...
async function getContributionCalendar(username) {
  ...
  if (calendar && calendar.totalContributions > 0) {
    console.log(
      `✅ Using GraphQL calendar: ${calendar.totalContributions} contributions`,
    );
    return calendar;
  }
  ...
  try {
    ...
    console.log(
      "⚠️ REST API fallback also limited, will use repository commit counts",
    );
  } catch (error) {
  ...
  console.log(
    "⚠️ Returning empty calendar - will use repository-based calculation",
  );
  return {
